...
async def my_proxy(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if my_proxy_num > 0:
        can_disable_auto_prolong = any(proxy.auto_prolong for proxy in response.list)
        can_enable_auto_prolong = any(not proxy.auto_prolong for proxy in response.list)
        download_menu = await download_proxies_keyboard(
            state,
            can_disable_auto_prolong=can_disable_auto_prolong,
            can_enable_auto_prolong=can_enable_auto_prolong,
        )
...
@router.callback_query(F.data == "enable_auto_prolong")
async def enable_auto_prolong(callback: CallbackQuery, state: FSMContext) -> None:
    await state.set_state(EnableAutoProlong.Choose)
    texts = await get_texts(state)
    await callback.answer()
    await safe_delete_message(callback)
    ...
    if not response.success:
        await state.update_data(enable_proxy_list=[])
        await state.set_state(None)
    ...
    disabled_auto_prolong_proxies = [proxy for proxy in response.list if not proxy.auto_prolong]
    proxy_list_raw = [proxy.model_dump() for proxy in disabled_auto_prolong_proxies]
    await state.update_data(enable_proxy_list=proxy_list_raw)

    if not proxy_list_raw:
        await state.set_state(None)
        empty_menu = await empty_proxy_menu(state)
        await callback.message.answer(
            text=texts['no_disabled_auto_prolong_proxies'],
            reply_markup=empty_menu,
        )
        return

    visible_proxies = disabled_auto_prolong_proxies[:20]
    keyboard = await proxy_checker_list(state, visible_proxies)

    await callback.message.answer(
        text=texts['choose_proxy_to_enable_auto_prolong'],
        reply_markup=keyboard,
    )


@router.message(EnableAutoProlong.Choose)
async def enable_auto_prolong_choose(message: Message, state: FSMContext) -> None:
    data = await state.get_data()
    texts = await get_texts(state)
    proxy_list_raw = data.get("enable_proxy_list", [])
    proxy_list = [ProxyItem(**p) for p in proxy_list_raw]
    visible_proxies = proxy_list[:20]
    keyboard = await proxy_checker_list(state, visible_proxies)
    ...
    proxy_api_client = ProxyAPIClient()
    response = await proxy_api_client.enable_auto_prolong(message.from_user.id, selected)
    ...
    if response.get("success"):
        ...
        await state.update_data(enable_proxy_list=updated_proxy_list)

        success_text = texts["auto_prolong_enabled"].format(proxy=selected)
        await message.answer(text=success_text)
        ...
        error_text = response.get("error") or texts["auto_prolong_enable_failed"]
        await message.answer(text=error_text)
